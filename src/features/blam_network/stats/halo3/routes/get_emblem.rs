...
fn draw_emblem(emblem: &Emblem, size: usize) -> RgbaImage {
    ...
    let colored_primary = get_colored_layer("PRIMARY", &primary_image, emblem.primary_color, size);
    let colored_secondary = get_colored_layer("SECONDARY", &primary_image, emblem.secondary_color, size);
    let colored_background = get_colored_layer("BACKGROUND", &background_image, emblem.background_color, size);
