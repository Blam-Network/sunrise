...
#[axum::debug_handler]
pub async fn get_reach_machine(Path((title, _, _, _, machine_id)): Path<(String, String, String, String, String)>) -> impl IntoResponse {
    let mut user = user_file {
        _blf: s_blf_chunk_start_of_file::new("omaha machine", byte_order_mark::default()),
